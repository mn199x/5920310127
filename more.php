...
$drvid =$_POST["drvid"];
	...
	$sql = "SELECT count(*) as np FROM book,dayscheduled 
			WHERE dayscheduled.slotid = book.slotid And book.bkdate = dayscheduled.schdate AND bkdate='$schdate' 
			AND book.slotid='$slotid' AND bstatus='1' AND drvid='$drvid'"; //FIXED this, as missing as
    // echo $sql;
...
$sql = "SELECT bkid,con_nme,con_tel,plcnme,numpgr, lik  FROM book,dayscheduled 
		WHERE dayscheduled.slotid = book.slotid AND book.bkdate = dayscheduled.schdate AND bkdate='$schdate' 
		AND book.slotid='$slotid' AND bstatus='1' AND drvid='$drvid'";
// echo $sql;			
$result = $conn->query($sql);
	
echo "<h2 style='color: #001a4d'>วันที่ ".$schdate."</h2>";
