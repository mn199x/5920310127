...
//count all patients from database order by name, this is used for caculate the numbers of pages
	$sql = "SELECT count(*) np FROM staff";
...
$sql = "SELECT stfid, stfnme, stfsurnme, s_email, stfsex, stftel
			FROM staff 
...
// echo "<th>ดำเนินการ</th>";
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			echo "<td>".$row["stfsex"]."</td>"; //convert gender id to string
			echo "<td>".$row["stftel"]."</td>";
			// echo "<td>";
			...
			// echo "<input type='hidden' name ='stfid'  value = '".$row["stfid"]."'/>";
...
echo "<tr><td colspan='13'><center>";
	echo"<form action = 'showSTF.php' method ='post'> ";
	echo "<input type='hidden' name ='stfid'  value = '$stfid'/>";		
	echo "<input type='hidden' name ='stfnme'  value = '$stfnme'/>";		
	echo "<input type='hidden' name ='stfsurnme' value = '$stfsurnme'/>";
	echo "<input type='hidden' name ='s_email' value = '$s_email'/>";
	echo "<input type='hidden' name ='stfsex'  value = '$stfsex'/>";
	echo "<input type='hidden' name ='stftel'  value = '$stftel'/>";
